...
covid19.jh <- covid19.jh.c %>% 
  ...
  mutate(Confirmed = ifelse(Country.Region=='Peru' & Report.Date=='2020-03-20',263,Confirmed),
         ...
         Two.days.inf.rate = Confirmed / lag(Confirmed, order_by = Report.Date, n = 2) -1,
         Two.days.inf.rate = ifelse(is.na(Two.days.inf.rate),0,Two.days.inf.rate),
         Five.days.inf.rate = Confirmed / lag(Confirmed, order_by = Report.Date, n = 5) -1,
         Five.days.inf.rate = ifelse(is.na(Five.days.inf.rate),0,Five.days.inf.rate),
...
covid19.jh %>% 
  ...
  mutate(Death.Rate = scales::percent(Death.Rate),
         ...
         Two.days.inf.rate = scales::percent(Two.days.inf.rate),
         Five.days.inf.rate = scales::percent(Five.days.inf.rate)) %>% 
  select(Report.Date, Country.Region, Confirmed, Recovered, Deaths, Actives, 
         Two.days.inf.rate, Five.days.inf.rate) %>% 
...
covid19.jh %>% 
  ...
  mutate(Death.Rate = scales::percent(Death.Rate),
         Recov.Rate = scales::percent(Recov.Rate)) %>% 
  select(Report.Date, Country.Region, New.Cases, New.Deaths, Death.Rate, Recov.Rate) %>% 
...
# Resumen
covid19.jh %>% 
  filter(Country.Region %in% c('US','Spain','Italy','United Kingdom','France','Serbia') &
           Report.Date == Latest.Date) %>% 
  mutate(Death.Rate = scales::percent(Death.Rate),
         Recov.Rate = scales::percent(Recov.Rate),
         Two.days.inf.rate = scales::percent(Two.days.inf.rate),
...
```{r, echo=FALSE, message=FALSE}
covid19.jh %>% 
  filter(Confirmed > 0 & Country.Region %in% c('US','Spain','Italy','United Kingdom','France','Serbia')) %>% 
  group_by(Country.Region) %>% 
  arrange(Country.Region, Report.Date) %>% 
  mutate(Dia = row_number(Country.Region)) %>% 
  select(Report.Date, Country.Region, Dia, everything()) %>% 
