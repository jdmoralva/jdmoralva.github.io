...
p <- covid19.jh %>% 
  ...
  ggtitle("Tendencia a través del tiempo COVID-19") +
...
```{r, echo=FALSE, message=FALSE}
covid19.jh %>% 
  filter(Confirmed > 0 & Country.Region %in% c('Peru','Ecuador','Colombia','Chile','Brazil','Venezuela')) %>% 
  group_by(Country.Region) %>% 
  ...
  geom_line(size=1) + 
  #geom_point(size=3, shape=21, fill="white") +
  ggtitle("Evolución en días después del primer contagio de COVID-19") +
  geom_dl(aes(label = Country.Region), method = list("last.points",cex=0.8,vjust=-0.8, hjust=0.5)) +
  theme_minimal() + 
