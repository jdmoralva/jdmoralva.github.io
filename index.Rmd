...
Información tomada de: http://query.data.world/s/cdc5dussbvmp52xnluz6wvyntjn6am
...
# Ajustes Varios
# Report Date
names(covid19)[1]="Table.Names"
names(covid19)[5]="Report_Date"

# Ajustes Info Peru
covid19 <- covid19 %>% 
  select(Report_Date, everything()) %>% 
  mutate(Report_Date = as.Date(Report_Date, '%m/%d/%y'),
         Cases = ifelse(Country_Region=='Peru' & Report_Date=='2020-03-20' & 
                          Case_Type =='Confirmed',263,Cases),
         Difference = ifelse(Country_Region=='Peru' & Report_Date=='2020-03-20' & 
                               Case_Type =='Confirmed',29,Difference),
         Cases = ifelse(Country_Region=='Chile' & Report_Date=='2020-03-19' & 
                          Case_Type =='Confirmed',342,Cases),
         Difference = ifelse(Country_Region=='Chile' & Report_Date=='2020-03-19' & 
                               Case_Type =='Confirmed',104,Difference))
...
```{r, echo=FALSE}
covid19 %>% 
  select(-Prep_Flow_Runtime, -Province_State, -Lat, -Long, -Table.Names, -Admin2, -FIPS, -Combined_Key) %>% 
...
p <- covid19 %>% 
  select(-Prep_Flow_Runtime, -Province_State, -Lat, -Long, -Table.Names) %>% 
  select(Latest_Date, everything(), -Difference) %>% 
  mutate(Report_Date = as.Date(Report_Date, '%m/%d/%y'),
         Latest_Date = as.Date(Latest_Date, '%m/%d/%y')) %>% 
  filter(Country_Region %in% c('Peru','Ecuador','Colombia','Chile','Brazil','Venezuela') & 
