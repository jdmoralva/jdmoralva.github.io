...
q <- covid19.jh %>% 
  ...
  ggtitle("Evolución de los nuevos Casos Confirmados de COVID-19") +
  theme_minimal()
...
covid19.jh %>% 
  ...
  geom_line(size=1) + 
  #geom_point(size=3, shape=21, fill="white") +
  ggtitle("Evolución de casos confirmados días después del primer contagio de COVID-19") +
  geom_dl(aes(label = Country.Region), method = list("last.points",cex=0.8,vjust=-0.8, hjust=0.5)) +
  theme_minimal() + 
...
covid19.jh %>% 
  ...
  geom_line(size=1) + 
  #geom_point(size=3, shape=21, fill="white") +
  ggtitle("Evolución de la tasa de mortalidad días despúes del primer contagio de COVID-19") +
  geom_dl(aes(label = Country.Region), method = list("last.points",cex=0.8,vjust=-0.8, hjust=0.5)) +
  theme_minimal() + 
  theme(legend.position="none",
          axis.title = element_blank())
...
covid19.jh %>% 
  ...
  mutate(Dia = row_number(Country.Region)) %>% 
  select(Report.Date, Country.Region, Dia, everything()) %>% 
  ggplot(aes(y = Recov.Rate, x = Dia, group=Country.Region, color=Country.Region)) +
  geom_line(size=1) + 
  #geom_point(size=3, shape=21, fill="white") +
  ggtitle("Evolución de la tasa de recupero días despúes del primer contagio de COVID-19") +
  geom_dl(aes(label = Country.Region), method = list("last.points",cex=0.8,vjust=-0.8, hjust=0.5)) +
  theme_minimal() + 
  theme(legend.position="none",
          axis.title = element_blank())
